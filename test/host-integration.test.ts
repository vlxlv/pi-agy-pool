...
test("CP4 cwd propagation and idle replacement",async()=>{
 ...
 const history:any[]=[];
 for(const cwd of ["/workspace/a","/workspace/a","/workspace/b"]){const before=children.length;const stream=streamSimple(model,{messages:[...history,...context.messages]} as any,{sessionId:"cwd",cwd,spawnFn} as any);const c=children.at(-1);if(children.length>before)c.send({event:"init",conversation_id:"X"});await tick();c.result();history.push(...context.messages,await stream.result());}
...
test("CP4 reused result-only turns always start; HTTP callback inapplicable",async()=>{let invoked=0;const child=new FakeAgy();const history:any[]=[];for(let i=0;i<2;i++){const s=streamSimple(model,{messages:[...history,...context.messages]} as any,{sessionId:"reuse",spawnFn:(()=>child) as any,onResponse:async()=>{invoked++;throw Error("must not invoke")}});if(!i)child.send({event:"init",conversation_id:"X"});await tick();child.result();const answer=await s.result();assert.equal(answer.stopReason,"stop");history.push(...context.messages,answer);const events=await collect(s);assert.equal(events[0].type,"start");assert.equal(events.filter(e=>e.type==="start").length,1);}assert.equal(invoked,0);});
